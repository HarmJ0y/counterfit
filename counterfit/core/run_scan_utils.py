...
def get_printable_batch(target, samples, sample_index=None, is_result=False):
    if target.model_data_type == "text":
        result = samples

    elif target.model_data_type == "image":
        result = []
    ...
    elif target.model_data_type in ("pe", "html"):
        result = []
        target_path = config.targets_path + "/" + target.model_name
        if is_result and not hasattr(target.active_attack.sample_index, "__iter__"):
...
def get_run_summary(target, attack=None):
    ...
    if target.model_data_type == "text":
        ...
        rel_distance = [d / len(ii0) for d, ii0 in zip(distances, i_0)] * 100.
    elif target.model_data_type in ("pe", "html"):
        # change in file size
        metric = 'filesize'
        rel_distance = [len(iif) for iif in i_f]
        ...
        rel_distance = np.sqrt(np.nansum(np.square(i_f - i_0), axis=1)) / (np.linalg.norm(i_0, axis=1) + eps) * 100.
    else:
        raise ValueError("Unexpected model_data_type")

    result = (
        attack.results["final"]["images"]
        if target.model_data_type in ("image", "pe", "html")
...
def get_printable_run_summary(summary):
    ...
    # Print elapsed_time and queries hit
    elapsed_time_str = f"{summary['elapsed_time']:.1f}"
    query_rate_str = f"{summary['queries']:.0f} ({query_rate:.1f} {units})"
    output += f"[+] Elapsed time [sec] {elapsed_time_str} \n"
    output += f"\n[+] Total number of queries [rate] {query_rate_str} \n"        
    ...
    for name, summary in summaries_by_attack.items():
        ...
        if results_width > 0:
            if summary["best_params"] is not None:
                ...
                param_str = "N/A"
            data_list.append([name, summary["total_runs"], successes, best, param_str])
        else:
            data_list.append([name, summary["total_runs"], successes, best])
    st = SimpleTable(columns)
    output += '\n' + st.generate_table(data_list, row_spacing=0) + '\n'
